...
const BEER_LOG_LINES = [
  ...
  'ZEUS THROWS A CELEBRATORY THUNDERBOLT — A GENTLE ONE, FULL OF LOVE',
...
function _chugFinishApplyWin(xpMult, isSafe){
  ...
  if(xpMult > 0){
    ...
    if(xpMult >= 3)      { addLog('🏆 LEGENDARY — '+t.n+' ×'+xpMult+' = +'+g+' XP'); toast('🏆 LEGENDARY! +'+g+' XP'); floatText('LEGENDARY', '#ffd24a'); setTimeout(()=>epicToast(PERFECT_CHUG_LINES[Math.floor(Math.random()*PERFECT_CHUG_LINES.length)]), 800); }
    else if(xpMult >= 2) { addLog('⚡ PERFECT CHUG — '+t.n+' ×'+xpMult+' = +'+g+' XP'); toast('⚡ ×'+xpMult+'! +'+g+' XP'); setTimeout(()=>epicToast(PERFECT_CHUG_LINES[Math.floor(Math.random()*PERFECT_CHUG_LINES.length)]), 800); }
